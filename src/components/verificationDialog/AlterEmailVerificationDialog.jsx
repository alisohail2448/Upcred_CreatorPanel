...
const AlterEmailVerificationDialog = ({setAlterEmailVerify,setAlterEmailVerifyBtn ,open = true, onClose }) => {
...
//   const { platforms } = useAllPlatforms();
  ...
  const handleSendOtp = () =>{
    MockApi(() => {
      if (!sendOtp) {
        enqueueSnackbar("Code Sent Successfully on your mail", { variant: "success" });
        setSendOtp(true);
      }
      else {
        setSendOtp(false);
  ...
  const handleVerifiedOtp = () =>{
    ...
    setAlterEmailVerifyBtn(false)
    setAlterEmailVerify(true)
    onClose()
  }
...
export default AlterEmailVerificationDialog;
