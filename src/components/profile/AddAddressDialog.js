...
import { addCreatorAddress } from "src/services/creatorRequests";

const AddAddressDialog = ({ open = true, onClose }) => {
  ...
  const onSubmitPress = async() => {
    ...
    const res = await addCreatorAddress(formData)
    if (res.response_message === API_RESPONSE_MESSAGE.SUCCESS) {
      enqueueSnackbar(MESSAGES.SUCCESS.CREATOR_ADDRESS_ADD_SUCCESS, { variant: "success" });
      onClose();
    } else {
      enqueueSnackbar(MESSAGES.ERROR.CREATOR_ADDRESS_ADD_FAILURE, { variant: "error" });
    }
  };
